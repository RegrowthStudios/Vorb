macro(setup_components)
    ...
    setup_root()
    setup_core()
    SET(VORB_LIBRARY_FILES ${VORB_ROOT_FILES} ${VORB_CORE_FILES})

    # Setup Other Components
    IF (VORB_BUILD_GRAPHICS)
        setup_graphics()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_GRAPHICS_FILES})
    ENDIF (VORB_BUILD_GRAPHICS)
    IF (VORB_BUILD_IO)
        setup_io()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_IO_FILES})
    ENDIF (VORB_BUILD_IO)
    IF (VORB_BUILD_PLATFORM)
        setup_platform()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_PLATFORM_FILES})
    ENDIF (VORB_BUILD_PLATFORM)
    IF (VORB_BUILD_SOUND)
        setup_sound()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_SOUND_FILES})
    ENDIF (VORB_BUILD_SOUND)
    IF (VORB_BUILD_UI)
        setup_ui()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_UI_FILES})
    ENDIF (VORB_BUILD_UI)
    IF (VORB_BUILD_VOXELS)
        setup_voxels()
        LIST(APPEND VORB_LIBRARY_FILES ${VORB_VOXELS_FILES})
