...
namespace vorb {
     ...
     *  
     *  With the exception of NONE and ALL, each value is a single unique bit that acts as a flag.
     *  Thus combinations such as (GRAPHICS | IO) can be created and used.
     */
    ...
    /*!
     * \fn InitParam init(const InitParam& p);
     *
     * \brief Initialize a set of Vorb subsystems.
     *  
     *  If a subsystem is already initialized, then this has no effect on that subsystem and it
    ...
    /*!
     * \fn InitParam dispose(const InitParam& p);
     *
     * \brief Dispose a set of Vorb subsystems.
     *  
     *  If a subsystem is not initialized, then this has no effect on that subsystem and it returns
     *  as a success.
     *
