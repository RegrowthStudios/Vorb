...
namespace vorb {
    namespace script {
        namespace lua {
            // Defines the signature of functions callable directly from Lua.
            using Handle         = lua_State*;
            using ScriptFunction = int(*)(Handle);
            ...
            namespace impl {
                ...
                inline i32 handleInvocation(Handle state, Delegate<void>* delegate) {
                    // Just invoke delegate, no parameters to fetch off lua stack.
                    delegate->invoke();
                ...
                /*!
                 * \brief This function acts as a wrapper, grabing the captured delegate from the Lua stack and handing off for invocation.
                 *
