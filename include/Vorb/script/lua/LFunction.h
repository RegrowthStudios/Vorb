...
namespace vorb {
    namespace script {
        namespace lua {
            ...
            // Forward declaration of SRLFunction.
            ...
            public:
                 ...
                 *
                 * \param lFunction The RLFunction instance to copy.
                 */
                LFunction(const LFunction& lFunction) :
                ...
                /*!
                 * \brief Creates an SRLFunction wrapper, which expects the wrapped lua function to return the type of ReturnType.
                 *
                 ...
                 */
                template<typename ReturnType>
                SRLFunction<ReturnType> withSingleReturn() const;
            ...
            /*! \brief Variant of the wrapper that specifies a single expected return type. */
            template<typename ReturnType>
            class SRLFunction : public LFunction {
            public:
                SRLFunction() :
                    LFunction()
                { /* EMPTY */ }
                 ...
                 * \param index The index of the function in the Lua registry.
                 */
                SRLFunction(Handle state, const nString& name, Index index) :
                 ...
                 */
                template<typename ...Parameters>
                ReturnType call(Parameters... parameters) const;
                /*!
                 * \brief Functions to call the wrapped lua function.
                 *
                 * operator() is an alias of call.
...
template<typename ReturnType>
inline vscript::lua::SRLFunction<ReturnType> vscript::lua::LFunction::withSingleReturn() const {
    return SRLFunction<ReturnType>(*this);
}
...
template<typename ReturnType>
template<typename ...Parameters>
inline ReturnType vscript::lua::SRLFunction<ReturnType>::call(Parameters ...parameters) const {
    // Put the script function table in the lua registry on to the top of the lua stack.
    lua_getfield(m_state, LUA_REGISTRYINDEX, SCRIPT_FUNCTION_TABLE);
    // Get a raw accessor to the wrapped script function.
    lua_rawgeti(m_state, -1, m_index);
    ...
    if (!ValueMediator<ReturnType>::tryPop(m_state, returnVal)) {
        // TODO(Matthew): Unify error handling across the environment?
        // Handle the failure.
...
template<typename ReturnType>
template<typename ...Parameters>
inline ReturnType vscript::lua::SRLFunction<ReturnType>::operator()(Parameters ...parameters) const {
    return call(std::forward<Parameters>(parameters)...);
}
