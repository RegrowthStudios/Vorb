...
bool vui::InputMapper::getInputState(const InputID id) {
    // Check Input
    if (id < 0 || id >= (int)m_inputs.size()) return false;
    return m_keyStates[m_inputs.at(id).key];
}

vui::InputMapper::InputID vui::InputMapper::createInput(const nString& inputName, VirtualKey defaultKey) {
...
vui::InputMapper::InputID vui::InputMapper::getInputID(const nString& inputName) const {
...
void vui::InputMapper::loadInputs(const nString &location /* = INPUTMAPPER_DEFAULT_CONFIG_LOCATION */) {
...
void vui::InputMapper::startInput() {
    if (!m_receivingInput) {
        vui::InputDispatcher::mouse.onButtonDown += makeDelegate(*this, &vui::InputMapper::onMouseButtonDown);
        vui::InputDispatcher::mouse.onButtonUp += makeDelegate(*this, &vui::InputMapper::onMouseButtonDown);
        vui::InputDispatcher::key.onKeyDown += makeDelegate(*this, &vui::InputMapper::onKeyDown);
        vui::InputDispatcher::key.onKeyUp += makeDelegate(*this, &vui::InputMapper::onKeyUp);
        m_receivingInput = true;
    }
}
void vui::InputMapper::stopInput() {
    if (m_receivingInput) {
        vui::InputDispatcher::mouse.onButtonDown -= makeDelegate(*this, &vui::InputMapper::onMouseButtonDown);
        vui::InputDispatcher::mouse.onButtonUp -= makeDelegate(*this, &vui::InputMapper::onMouseButtonDown);
        vui::InputDispatcher::key.onKeyDown -= makeDelegate(*this, &vui::InputMapper::onKeyDown);
...
VirtualKey vui::InputMapper::getKey(const InputID id) {
    if (id < 0 || id >= (int)m_inputs.size()) return VKEY_HIGHEST_VALUE;
    return m_inputs.at(id).key;
}

void vui::InputMapper::setKey(const InputID id, VirtualKey key) {
...
void vui::InputMapper::setKeyToDefault(const InputID id) {
...
void vui::InputMapper::onKeyUp(Sender, const vui::KeyEvent& e) {
