...
vui::Viewport* vui::UIBase::makeViewFromYAML(nString name, ZIndex zIndex, nString filepath) {
...
vui::Viewport* vui::UIBase::getView(nString name) {
    for (auto& view : m_views) {
...
bool vui::UIBase::destroyViewWithName(nString name) {
    for (auto it = m_views.begin(); it != m_views.end(); ++it) {
        auto& view = *it;
        if (view.second->getName() == name) {
            view.second->dispose();
...
bool vui::UIBase::setViewWithNameZIndex(nString name, ZIndex zIndex) {
    for (auto it = m_views.begin(); it != m_views.end(); ++it) {
        auto& view = *it;
        if (view.second->getName() == name) {
